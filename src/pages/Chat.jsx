...
const FREE_TRIAL_MESSAGE_LIMIT = 3;
...
export default function Chat() {
    ...
    useEffect(() => {
        ...
        // Only load sessions for subscribed users
        if (isSubscribed) {
            loadSessions();
        } else {
    ...
    const handleSubmit = async (e) => {
        ...
        if (!isSubscribed && userMessageCount >= FREE_TRIAL_MESSAGE_LIMIT) {
            // Don't allow more messages, limit reached
            return;
        }
        ...
        try {
            ...
            if (response.ok) {
                ...
                // Free trial limit reached on backend
                const errorData = await response.json().catch(() => ({ error: 'Free trial limit reached' }));
                console.warn('Free trial limit enforced by backend:', errorData);
                // Remove the optimistically added user message
                setMessages(prev => prev.slice(0, -1));
                // The UI should already show the limit warning, backend is just enforcing it
            } else {
        ...
        <div className="fixed inset-0 flex overflow-hidden bg-white dark:bg-gray-950 pt-14 sm:pt-16">
            ...
            `}>
                <div className="flex flex-col h-full">
                    ...
                    {/* Sessions List or Subscription Banner */}
                            ...
                            <div className="p-4 space-y-3">
                                <div className="bg-gradient-to-br from-blue-500 to-purple-600 rounded-lg p-4 text-white">
                                    <div className="flex items-start gap-2 mb-2">
                                        ...
                                        <div>
                                            <h3 className="font-semibold text-sm mb-1">Free Trial</h3>
                                            <p className="text-xs opacity-90 mb-3">
                                                You can try up to 3 messages per conversation. Subscribe to unlock unlimited chat and save your conversation history.
                                            </p>
                                            <button
                                                onClick={() => navigate('/subscription')}
                                                className="bg-white text-blue-600 px-3 py-1.5 rounded-lg text-xs font-semibold hover:bg-gray-100 transition-colors"
                                            >
                                                Subscribe Now
                                            </button>
                                        </div>
                                ...
                                <div className="text-xs text-gray-500 dark:text-gray-400 text-center px-2">
                                    Your conversations are saved but hidden during the trial. They'll appear here once you subscribe.
                                </div>
                            </div>
                        ) : loadingSessions ? (
            ...
            <div className={`flex-1 flex flex-col min-w-0 overflow-hidden transition-all duration-300 ease-in-out ${showSidebar ? 'md:ml-64' : 'ml-0'}`}>
                ...
                <div className="border-t border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-950 flex-shrink-0 safe-area-inset-bottom">
                    <div className="max-w-3xl mx-auto p-3 md:p-4 w-full">
                        {/* Free trial limit warning */}
                        {!isSubscribed && userMessageCount >= FREE_TRIAL_MESSAGE_LIMIT && (
                                ...
                                <div className="flex items-start gap-2">
                                    ...
                                    <div className="flex-1">
                                        ...
                                        <p className="text-xs text-yellow-700 dark:text-yellow-300 mt-1">
                                            You've used all 3 messages in this conversation. Subscribe to continue chatting and unlock unlimited messages.
                                        </p>
                                        <button
                                            onClick={() => navigate('/subscription')}
                                            className="mt-2 text-xs font-semibold text-yellow-800 dark:text-yellow-200 hover:text-yellow-900 dark:hover:text-yellow-100 underline"
                                        >
                                            Subscribe now →
                                        </button>
                                    </div>
                        ...
                        {!isSubscribed && userMessageCount > 0 && userMessageCount < FREE_TRIAL_MESSAGE_LIMIT && (
                            <div className="mb-2 text-center">
                                <span className="text-xs text-gray-500 dark:text-gray-400">
                                    Free trial: {userMessageCount} / {FREE_TRIAL_MESSAGE_LIMIT} messages used
                                </span>
                            </div>
                        )}
                        ...
                        <form onSubmit={handleSubmit} className="relative">
                            <div className="relative flex items-end gap-2 bg-gray-100 dark:bg-gray-900 rounded-2xl border border-gray-200 dark:border-gray-800 focus-within:border-gray-300 dark:focus-within:border-gray-700 transition-colors p-2">
                                ...
                                <button
                                    type="submit"
                                    disabled={isLoading || !input.trim() || (!isSubscribed && userMessageCount >= FREE_TRIAL_MESSAGE_LIMIT)}
